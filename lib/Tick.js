...
export default class Tick {
  ...
  // increment by tick size and round to the nearest tick size
  incr(num) {
    return this.round(num + this.tickSize);
  };

  // decrement by tick size and round to the nearest tick size  
  decr(num) {
    return this.round(num - this.tickSize);
  };

  // increment by step size and round to the nearest step size
  incrStep(num) {
    return this.roundStep(num + this.stepSize);
  };

  // decrement by step size and round to the nearest step size
  decrStep(num) {
    return this.roundStep(num - this.stepSize);
  };

  // increment by x step size and round to the nearest step size
  addSteps(num, x) {
    return this.roundStep(num + x * this.stepSize);
  };

  // decrement by x step size and round to the nearest step size
  subSteps(num, x) {
    return this.roundStep(num - x * this.stepSize);
  };

  // parse and round to tick size
  parse(str) {
    return this.round(parseFloat(str));
  }

  // parse and round to step size
  parseStep(str) {
