...
export default class BinanceOrderBook {
  ...
  constructor(feed, symbol, tickSize, updateInterval = 250) {
    const self = this;
    this.tick = new Tick(tickSize);
    this.updateInterval = updateInterval; // 存储更新间隔
  ...
  onTrade(data) {
    ...
    const trade = {
      ...
      time: fmtTime(data.T, this.updateInterval), // 使用 updateInterval 参数,
      timestamp: data.T
    };
  ...
  updateOrderBook(snapshot) {
    ...
    this.bid = this.tick.parse(snapshot.bids[0][0]);
    this.ask = this.tick.parse(snapshot.asks[0][0]);
    this.order_delta = snapshot.order_delta;
  ...
  updateTrade(trade) {
    // 检查交易数据是否有效
    if (!trade || !trade.price || !trade.quantity) {
      console.warn('Invalid trade data:', trade);
      return;
    }
    
    // 解析价格和数量
    const price = this.tick.parse(trade.price);
    const quantity = this.tick.parse(trade.quantity);
    ...
    // 添加到交易列表
    this.onTrade(formattedTrade);
    
    // 更新统计数据
    this.updateTradeStats(formattedTrade);
  }
  ...
  updateTradeStats(trade) {
    ...
    this.stats.volume += trade.quantity;
    
    // 更新买卖方向的交易量
    if (trade.isBuyerMaker) {
      // 卖方发起的交易
      if (!this.stats.sellVolume) {
        this.stats.sellVolume = 0;
      }
      this.stats.sellVolume += trade.quantity;
    } else {
      // 买方发起的交易
      if (!this.stats.buyVolume) {
        this.stats.buyVolume = 0;
      }
      this.stats.buyVolume += trade.quantity;
    }
    
    // 更新最高价和最低价
    if (!this.stats.highPrice || trade.price > this.stats.highPrice) {
      this.stats.highPrice = trade.price;
    }
    if (!this.stats.lowPrice || trade.price < this.stats.lowPrice) {
      this.stats.lowPrice = trade.price;
    }
